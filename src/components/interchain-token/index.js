...
import { BiCheck, BiMessage } from "react-icons/bi";
...
import { ellipse, loaderColor, parseError, toArray } from "~/lib/utils";
import { TOKEN_ADDRESSES_DATA, TOKEN_LINKERS_DATA } from "~/reducers/types";
...
export default () => {
  ...
  const { chain_id, provider, web3_provider, address, signer } = {
    ...wallet_data,
  };
    ...
    if (
      ...
      try {
        ...
        const proxy = await deployAndInitContractConstant(
          ...
          _signer,
          callback ? (response) => callback(response) : undefined
        );
  ...
  const deployAndInitContractConstant = async (
    ...
    if (constant_address_deployer && _signer && key && contract_json) {
      ...
      const init_data = (await contract.populateTransaction.init(...init_args))
        ?.data;
      ...
      try {
        ...
        const transaction = await deployer
          .connect(_signer)
          .deployAndInit(bytecode, salt, init_data);
  ...
  const _deployTokenLinker = async (_signer = signer, callback) => {
    let response;
    ...
    if (
      ...
      const { token_linker_address, deployed } = { ...token_linker };
      ...
      const gateway_address = getContractAddressByChain(
        id,
        gateway_addresses_data
      );

      const gas_service_address = getContractAddressByChain(
        id,
        gas_service_addresses_data
      );
          ...
          try {
            ...
            if (!failed) {
              response = {
                ...response,
                remote_address_validator_address,
                deployed: await isContractDeployed(
                  token_linker_address,
                  InterchainTokenLinker,
                  _signer
                ),
  ...
  const deployTokenLinker = async (chain, _signer) => {
    ...
    if (chain_data?.chain_id !== chain_id && !_signer) {
      ...
      const _signer = await switchChain(
        chain_data?.chain_id,
        provider,
        evm_chains_data
      );
      ...
      const response = await _deployTokenLinker(_signer, (updated_status) => {
        setTokenLinkerDeployStatus({
          chain,
          ...updated_status,
        });
      ...
      const { deployed, status, message, code } = { ...response };

      setTokenLinkerDeployStatus(
        deployed || ["user_rejected"].includes(code)
          ? null
          : status === "failed"
          ? {
              ...response,
            ...
            <div className="w-full xl:px-1">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-4 lg:gap-6 xl:grid-cols-5">
                {getSupportedEvmChains()
                  ...
                  .map((tl, i) => {
                    const { chain_data, token_linker_address, deployed } = {
                      ...tl,
                    };
                    const { id, name, image, explorer } = { ...chain_data };
                    const { url, address_path } = { ...explorer };
                          ...
                          <div className="flex h-full flex-col justify-between space-y-5">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center justify-between space-x-1 rounded-lg border border-slate-100 py-1.5 pl-1.5 pr-1 dark:border-slate-800">
                                ...
                                {token_linker_address && (
                                  <Copy value={token_linker_address} />
                                )}
                              </div>
                            </div>
                            {deployed ? (
