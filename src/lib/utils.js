  ...
  if (
    ...
    if (formatted_number.includes("e-")) {
      ...
      if (Number(e) <= 72) {
        ...
        _number = Number(
          utils.formatUnits(BigInt(_number), 16 + fixed_decimals)
        );
    ...
    !format?.includes("a") &&
    Number(split(formatted_number).join("")).toString() !==
      removeDecimal(split(formatted_number).join(""))
  ) {
    formatted_number = number.toString();
  }
...
export const capitalize = (s) =>
  typeof s !== "string"
    ? ""
    : split(
        split(split(s, "normal", " ").join("_"), "normal", "-").join("_"),
        "normal",
        "_"
      )
