...
export default (
 ...
 const {
  ...
  useEffect(
    () => {
      if (evm_chains_data && token_linkers_data) {
        setMenus(evm_chains_data.filter(c => c?.id && !c.deprecated && c.gateway_address && token_linkers_data[c.id]?.deployed));
      }
    },
  ...
  return (
    menus &&
    (
      <div className={`${displayName ? "bg-white dark:bg-black" : "backdrop-blur-16"} w-40 shadow dark:shadow-slate-700 rounded-lg flex flex-col py-1`}>
        {menus
          .map((m, i) => {
            const {
              id,
            ...
            const className =
              `w-full ${i === 0 ? "rounded-t-lg" : i === menus.length - 1 ? "rounded-b-lg" : ""} ${
                disabled ?
                  "cursor-default" :
                  "cursor-pointer"
