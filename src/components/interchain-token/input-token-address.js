...
export default () => {
  ...
  useEffect(() => {
    const supported_evm_chains_data = toArray(evm_chains_data).filter(
      (c) =>
        c?.id &&
        c.chain_id &&
        !c.deprecated &&
  ...
  useEffect(() => {
    if (typeof input === "string") {
      try {
        ...
        router.push(
          `${pathname.replace("/[chain]", "").replace("/[token_address]", "")}${
            selectedChain && _input ? `/${selectedChain}/${_input}` : ""
          }`,
          undefined,
          {
            shallow: true,
    ...
    Object.values({ ...token_linkers_data }).filter((tl) => tl?.deployed)
      ...
      <div className="w-full sm:max-w-md border border-slate-300 dark:border-slate-700 focus:border-blue-500 dark:focus:border-blue-500 rounded-xl flex items-center justify-between space-x-2.5 py-2 px-3">
        <DebounceInput
          ...
          size="small"
          type="text"
          placeholder={`Search for an existing ERC-20 token address${
